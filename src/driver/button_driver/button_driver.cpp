...
struct ButtonState {
    const uint8_t pin;
    const button_event_t press_event;
    const button_event_t long_press_event;
    const button_event_t double_click_event;

    ButtonFsmState state = IDLE;
    unsigned long last_change_time = 0;
};

static ButtonState select_button = { 
    config.buttons.select_pin, 
    BTN_PRESS_SELECT, 
    BTN_LONG_PRESS_SELECT, 
    BTN_DOUBLE_CLICK_SELECT 
};

static ButtonState next_button = { 
    config.buttons.next_pin, 
    BTN_PRESS_NEXT, 
    BTN_LONG_PRESS_NEXT, 
...
void button_driver_task_fn(void* pv) {
    ...
    pinMode(next_button.pin, INPUT_PULLUP);

    for (;;) {
        update_button_fsm(select_button);
        update_button_fsm(next_button);
